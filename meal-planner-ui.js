(() => {
  ...
  function showRecipe(day,item){
    ...
    $('mpRecipeDetail').innerHTML='<div class="card mp-recipe-detail"><button class="secondary mp-recipe-close">Cerrar</button><span class="pill">'+esc(meal.slot||meal.recipe.m)+'</span><h2>'+esc(meal.recipe.n)+'</h2><div class="kcal">'+meal.k+' kcal</div><p class="small">P '+meal.p+' g · C '+meal.c+' g · G '+meal.f+' g'+(meta?' · '+meta:'')+'</p><div class="good">Las cantidades visibles son exactamente las usadas para calcular esta ración y encajarla en los objetivos diarios.</div><button class="mp-log-detail '+(logged?'secondary':'')+'" '+(logged?'disabled':'')+'>'+(logged?'✓ Registrada hoy':'Añadir al diario de hoy')+'</button><h3>Ingredientes y aporte nutricional</h3><ul>'+ingredients+'</ul><p class="small">El aporte por ingrediente es una estimación proporcional a la ficha nutricional completa de la receta; los totales mostrados son los utilizados por el plan.</p>'+preparation+'</div>';
