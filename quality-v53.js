...
function rebalanceDay(day,prefs={}){
  ...
  const items=hasFixed?day.items.map(item=>({...item})):day.items.map((item,i)=>rescaleItem(item,targetKcals(daily,shares)[i]));
  ...
  const planner=root?.RecompMealPlanner,withinTarget=planner?.withinTargets?planner.withinTargets(t,prefs,{k:.03,p:.05,c:.06,f:.08}):Math.abs(t.k-daily)/daily<=.03;
  return{...day,items,totals:t,withinTarget,macroAdjusted:false,energyDistribution:{version:'v53',shares:actual.map(x=>Number(x.toFixed(4))),targets,pattern,policy:hasFixed?'fixed-morning-first':'meal-budget-first'}};
}
function stabilizePlan(plan){if(!plan||!Array.isArray(plan.days))return plan;const prefs=plan.preferences||{};const days=plan.days.map(day=>rebalanceDay(day,prefs));return{...plan,days,quality:{...(plan.quality||{}),mealDistribution:'v53-meal-budget-first'}};}
function installPlannerGuard(){const planner=root?.RecompMealPlanner;if(!planner||planner.__qualityV53)return false;const originalGenerate=planner.generateDays?.bind(planner),originalSwap=planner.swapMeal?.bind(planner);if(typeof originalGenerate!=='function')return false;planner.generateDays=(recipes,input={})=>stabilizePlan(originalGenerate(recipes,input));planner.generate30Days=(recipes,input={})=>planner.generateDays(recipes,{...input,days:30});if(typeof originalSwap==='function')planner.swapMeal=(plan,recipes,dayIndex,itemIndex)=>stabilizePlan(originalSwap(plan,recipes,dayIndex,itemIndex));planner.rebalanceDayV53=rebalanceDay;planner.__qualityV53=true;return true;}
