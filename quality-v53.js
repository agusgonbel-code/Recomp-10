...
function rebalanceDay(day,prefs={},options={}){
  ...
  // Generation or substitution may already satisfy all four targets. Do not
...
function installPlannerGuard(){const planner=root?.RecompMealPlanner;if(!planner||planner.__qualityV53)return false;const originalGenerate=planner.generateDays?.bind(planner),originalSwap=planner.swapMeal?.bind(planner);if(typeof originalGenerate!=='function')return false;planner.generateDays=(recipes,input={})=>stabilizePlan(originalGenerate(recipes,input),{preserveNutrientFit:true});planner.generate30Days=(recipes,input={})=>planner.generateDays(recipes,{...input,days:30});if(typeof originalSwap==='function')planner.swapMeal=(plan,recipes,dayIndex,itemIndex)=>stabilizePlan(originalSwap(plan,recipes,dayIndex,itemIndex),{preserveNutrientFit:true});planner.rebalanceDayV53=rebalanceDay;planner.__qualityV53=true;return true;}
