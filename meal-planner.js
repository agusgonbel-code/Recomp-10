...
const slotsFor=count=>({1:[['Comida principal',1]],2:[['Comida',.55],['Cena',.45]],3:[['Desayuno',.25],['Comida',.40],['Cena',.35]],4:[['Desayuno',.22],['Comida',.34],['Merienda',.14],['Cena',.30]],5:[['Desayuno',.20],['Media mañana',.10],['Comida',.32],['Merienda',.12],['Cena',.26]],6:[['Desayuno',.18],['Media mañana',.10],['Comida',.28],['Merienda',.12],['Cena',.24],['Snack nocturno',.08]]})[count]||slotsFor(4);
const remainingSlotsAfterBreakfast=count=>({2:[['Comida',.55],['Cena',.45]],3:[['Comida',.42],['Merienda',.18],['Cena',.40]],4:[['Media mañana',.14],['Comida',.38],['Merienda',.14],['Cena',.34]],5:[['Media mañana',.12],['Comida',.34],['Merienda',.16],['Cena',.28],['Snack nocturno',.10]]})[count]||slotsFor(count);
...
function swapDayWithFixedMeals(plan,recipes,dayIndex,itemIndex){
  const day=plan.days[dayIndex],old=day.items[itemIndex],prefs=plan.preferences;
  if(!old)throw new Error('Comida no válida.');
  const isFixed=item=>/^fixed-/.test(String(item?.recipe?.id||''));
  // The selected fixed meal is unlocked only for this day. Other fixed meals
  // retain their exact ledgers and positions; profile settings stay untouched.
  const editable=day.items.map((item,index)=>({item,index})).filter(({item,index})=>index===itemIndex||!isFixed(item));
  const retained=day.items.filter((item,index)=>index!==itemIndex&&isFixed(item));
  const selected=editable.findIndex(entry=>entry.index===itemIndex),fixedTotal=totals(retained);
  const cap=({3:.45,4:.40,5:.36,6:.34,7:.32})[day.items.length]||.40;
  const target={...prefs,meals:editable.length,kcal:prefs.kcal-fixedTotal.k,protein:prefs.protein-fixedTotal.p,carbs:prefs.carbs-fixedTotal.c,fat:prefs.fat-fixedTotal.f,_maxItemKcal:prefs.kcal*(cap+.004)};
  if(['kcal','protein','carbs','fat'].some(key=>!Number.isFinite(target[key])||target[key]<=0))throw new Error('Las tomas restantes no permiten una sustitución equivalente.');
  const energy=editable.reduce((sum,{item})=>sum+item.k,0);
  const slots=editable.map(({item})=>[item.slot,item.k/energy]);target._slots=slots;
  const assemble=variable=>{const items=day.items.slice();editable.forEach(({index},i)=>{items[index]=variable[i]});return items;};
  const accept=variable=>{const items=assemble(variable),t=totals(items);return withinTargets(t,prefs,{k:.03,p:.05,c:.06,f:.08})&&items.every(item=>item.k>0&&item.k/Math.max(1,t.k)<=cap+.005)};
  const pool=allowedRecipes(recipes.map(withComposition),prefs).filter(recipe=>recipe.n!==old.recipe.n&&!isFixed({recipe}));
  const ranked=pool.map(recipe=>{
    const scale=clamp(old.k/Math.max(1,recipe.k),.18,5);
    const error=Math.abs(recipe.p*scale-old.p)/Math.max(10,old.p)+Math.abs(recipe.c*scale-old.c)/Math.max(15,old.c)+Math.abs(recipe.f*scale-old.f)/Math.max(6,old.f);
    return {recipe,score:error+(slotAliases(old.slot).includes(recipe.m)?0:1)};
  }).sort((a,b)=>a.score-b.score);
  let variable=null;
  for(const {recipe} of ranked){
    const chosen=editable.map(({item},index)=>index===selected?recipe:item.recipe);
    const fitted=optimizeIngredients(chosen,target,slots);
    if(accept(fitted)){variable=fitted;break;}
  }
  if(!variable)for(const [index,{recipe}] of ranked.slice(0,12).entries()){
    const forced={avoid:old.recipe.n,[selected]:recipe};
    const built=buildDay(pool,target,new Map(),dayIndex+index*17,forced,{primary:24,secondary:32,accept});
    if(accept(built.items)){variable=built.items;break;}
  }
  if(!variable)throw new Error('No se encontró una alternativa que conserve tus macros y el reparto. El menú anterior se conserva.');
  const items=assemble(variable),t=totals(items),fixedMealSubstitutions={...(day.fixedMealSubstitutions||{})};
  if(isFixed(old))fixedMealSubstitutions[itemIndex]=old.recipe.id;
  plan.days[dayIndex]={...day,items,totals:t,error:macroError(t,prefs),withinTarget:true,macroAdjusted:false,fixedMealSubstitutions};
  return plan;
}
function swapMeal(plan,recipes,dayIndex,itemIndex){if(!plan?.days?.[dayIndex])throw new Error('Día no válido.');if(plan.days[dayIndex].fixedMealSubstitutions||plan.days[dayIndex].items.some(item=>/^fixed-/.test(String(item?.recipe?.id||''))))return swapDayWithFixedMeals(plan,recipes,dayIndex,itemIndex);const prefs=validatePreferences({...plan.preferences,days:plan.days.length}),pool=allowedRecipes(recipes.map(withComposition),prefs),day=plan.days[dayIndex],old=day.items[itemIndex];if(/^fixed-/.test(String(old?.recipe?.id||'')))throw new Error('Esta toma está fijada en tu perfil. Edita su configuración para cambiarla.');const fixed=day.items.filter(item=>/^fixed-/.test(String(item?.recipe?.id||''))),variable=day.items.filter(item=>!/^fixed-/.test(String(item?.recipe?.id||''))),variableIndex=variable.indexOf(old),remainingMeals=variable.length,remainingSlots=fixed.length?remainingSlotsAfterBreakfast(remainingMeals):slotsFor(remainingMeals),fixedTotals=totals(fixed),target=fixed.length?{...prefs,meals:remainingMeals,_slots:remainingSlots,kcal:prefs.kcal-fixedTotals.k,protein:prefs.protein-fixedTotals.p,carbs:prefs.carbs-fixedTotals.c,fat:prefs.fat-fixedTotals.f}:prefs;let candidates=pool.filter(r=>slotAliases(old.slot).includes(r.m)&&r.n!==old.recipe.n);if(!candidates.length)candidates=pool.filter(r=>r.n!==old.recipe.n);if(!candidates.length)throw new Error('No hay una alternativa equivalente disponible.');const ranked=candidates.map(r=>{const s=clamp(old.k/Math.max(1,r.k),.2,5),e=Math.abs(r.p*s-old.p)/Math.max(10,old.p)+Math.abs(r.c*s-old.c)/Math.max(15,old.c)+Math.abs(r.f*s-old.f)/Math.max(6,old.f);return {r,e};}).sort((a,b)=>a.e-b.e).slice(0,48),acceptSwap=items=>{const t=totals(items),cap=items.length>=5?.38:.40;return withinTargets(t,prefs,{k:.05,p:.06,c:.06,f:.07})&&items.every(item=>item.k/Math.max(1,t.k)<=cap+.005)};let best=null;ranked.forEach(({r})=>{const chosen=variable.map((x,i)=>i===variableIndex?r:x.recipe),optimized=optimizeIngredients(chosen,target,remainingSlots),variableItems=fixed.length?reconcileRemainingItems(optimized,target):optimized,items=[...fixed,...variableItems],t=totals(items),err=macroError(t,prefs),cap=items.length>=5?.38:.40,maxShare=Math.max(...items.map(item=>item.k/Math.max(1,t.k))),accepted=acceptSwap(items);if(fixed.length&&(maxShare>cap+.005||!withinTargets(t,prefs,{k:.03,p:.05,c:.06,f:.08})))return;if(!best||(accepted&&!best.accepted)||(accepted===best.accepted&&err<best.err))best={items,t,err,accepted};});if(!fixed.length&&!best?.accepted){for(const [candidateIndex,{r}] of ranked.slice(0,12).entries()){const forced={avoid:old.recipe.n};forced[variableIndex]=r;const built=buildDay(pool,prefs,new Map(),dayIndex+candidateIndex*17,forced,{primary:24,secondary:32,accept:acceptSwap}),accepted=acceptSwap(built.items);if(!accepted)continue;const t=totals(built.items),err=macroError(t,prefs);if(!best||!best.accepted||err<best.err)best={items:built.items,t,err,accepted:true};if(err<=.02)break;}}if(!best)throw new Error('No se encontró una alternativa que conserve tus macros y un reparto seguro. Prueba otra comida o ajusta el objetivo.');plan.days[dayIndex]={...day,items:best.items,totals:best.t,error:best.err,withinTarget:withinTargets(best.t,prefs,{k:.03,p:.05,c:.06,f:.08}),macroAdjusted:false};return plan;}
