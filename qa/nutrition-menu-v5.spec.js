...
test('nutrition is a daily dashboard, not a calculator-first screen', async ({ page }) => {
  ...
  await expect(page.locator('#nv5NutritionHeader')).toBeVisible();
  await expect(page.getByText('Come con un objetivo claro')).toBeVisible();
  await expect(page.locator('#nutricion .nv5-segment')).toBeVisible();
  await expect(page.locator('#nv5TodayMealsMirror')).toContainText('Desayuno QA');
  await expect(page.locator('#nv5NutritionHeader')).toContainText('500');
  await page.locator('#nutricion .nv5-segment button[data-panel="targets"]').click();
  await expect(page.getByText('Calcular objetivos')).toBeVisible();
  await page.locator('#nutricion .nv5-segment button[data-panel="diary"]').click();
...
test('menu is a single planning studio without duplicate quick generators', async ({ page }) => {
  ...
  await expect(page.locator('#nv5MenuTop')).toBeVisible();
  await expect(page.getByText('Construye una semana que puedas repetir.')).toBeVisible();
  ...
  await expect(page.locator('#nv5MenuTop')).toContainText('7 días planificados');
